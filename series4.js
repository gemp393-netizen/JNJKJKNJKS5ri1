...
function syncWatchedState(currentS, currentE) {
    ...
    SERIE.seasons.forEach((s, sIdx) => {
        s.episodes.forEach(e => {
            ...
            setWatched(sIdx, e.num, !isFuture);
        });
    });
}
...
function closePlayer() {
    renderCount++; // Invalida renderizados asíncronos pendientes
...
(function () {
    ...
    if (rBtn) rBtn.addEventListener('click', () => {
        if (!currentEpisode) return;
        ls.innerHTML = ''; (currentEpisode.langs || []).forEach((l, i) => { const opt = document.createElement('option'); opt.value = i; opt.textContent = l.name; if (i === activeLang) opt.selected = true; ls.appendChild(opt); });
        function fS(li) { ss.innerHTML = ''; (currentEpisode.langs?.[li]?.servers || []).forEach((s, i) => { const opt = document.createElement('option'); opt.value = i; opt.textContent = s.name; if (li === activeLang && i === activeServer) opt.selected = true; ss.appendChild(opt); }); }
        fS(activeLang); ls.onchange = () => fS(+ls.value); ts.value = ''; co.value = ''; st.textContent = ''; fv.style.display = ''; sv.style.display = 'none'; si.style.opacity = '0'; o.style.display = 'flex';
        requestAnimationFrame(() => { b.style.transform = 'scale(1)'; b.style.opacity = '1'; });
    });
    if (cBtn) cBtn.addEventListener('click', closeModal);
    if (o) o.onclick = e => { if (e.target === o) closeModal(); };
    if (sBtn) sBtn.onclick = async () => {
        const cfg = window.REPORT_CONFIG || {};
        if (!cfg.botToken || cfg.botToken === 'TU_BOT_TOKEN_AQUI') { st.style.color = '#ff5050'; st.textContent = 'Bot no configurado'; return; }
        if (!ts.value) { st.style.color = '#ff5050'; st.textContent = 'Selecciona problema'; return; }
        const li = +ls.value, sidx = +ss.value, lang = currentEpisode.langs?.[li]?.name || '-', srv = currentEpisode.langs?.[li]?.servers?.[sidx]?.name || '-';
        ...
        sBtn.disabled = true; st.textContent = 'Enviando...';
        try {
            const res = await fetch(`https://api.telegram.org/bot${cfg.botToken}/sendMessage`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chat_id: cfg.chatId, text: lines, parse_mode: 'Markdown', message_thread_id: cfg.topicId }) });
            const d = await res.json(); if (d.ok) { fv.style.display = 'none'; sv.style.display = ''; si.style.opacity = '1'; setTimeout(closeModal, 2000); } else throw 1;
        } catch { st.style.color = '#ff5050'; st.textContent = 'Error'; } finally { sBtn.disabled = false; }
    };
})();
