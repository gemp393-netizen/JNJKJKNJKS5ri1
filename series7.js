...
function syncWatchedState(sIdx, eNum) {
    if (localStorage.getItem('auto_watched') !== '1') return;
    SERIE.seasons.forEach((s, si) => s.episodes.forEach(e => {
        const f = (si > sIdx) || (si === sIdx && e.num > eNum), v = !f;
        setWatched(si, e.num, v);
        const i = document.querySelector(`.ep-switch[data-s="${si}"][data-e="${e.num}"] input`);
        if (i) i.checked = v;
        const lb = $(`lbl-${si}-${e.num}`);
        if (lb) { lb.textContent = v ? 'Visto' : 'No visto'; lb.classList.toggle('on', v); }
    }));
}
...
function playEpisode(seasonIdx, epNum, animate = false) {
    ...
    // Marcar como visto y sincronizar anteriores
    ...
    // Mostrar reproductor y ocultar elementos de navegación
    $('player-section').style.display = 'flex';
    $('episodes-list').style.display = 'none';
    document.querySelector('.seasons-wrap').style.display = 'none';
    const mainHeader = $('serie-header');
    if (mainHeader) mainHeader.style.display = 'none';

    // Actualizar título
    const baseTitle = currentEpisode.title.toLowerCase().startsWith('episodio') ? currentEpisode.title : `Ep. ${epNum} · ${currentEpisode.title}`;
    $('player-ep-title').textContent = baseTitle;
